   ...
   <div class="w3-modal-content w3-card-8 w3-animate-zoom" style="max-width:350px">
      ...
      <form class="w3-container" method="post">
        <div class="w3-section">
          ...
          <input class="w3-input w3-border w3-margin-left w3-margin-bottom" type="text" style="width:300px" placeholder="Enter Username" name="eusername" required>
          <label class="w3-margin-left"><b>Password</b></label>
          <input class="w3-input w3-border w3-margin-left" type="password" style="width:300px" placeholder="Enter Password" name="epassword" required>
          <div style="display:none" id="error msg" class="w3-panel w3-margin-left w3-margin-right w3-red">
           <h5>Invalid EUsername and EPassword!!</h5>
          </div> 
		  <button class="w3-btn-block w3-green w3-margin-left w3-section w3-padding" name="adminSubmit" style="width:300px" type="submit">Login</button>
        </div>
      </form>
    </div>
 </div>
...
<?php
 if($_SERVER["REQUEST_METHOD"] == "POST") {
  if (isset($_POST['adminSubmit'])) {
   require("config.php");
   $query  = $mysql_connect->query("SELECT eusername, epassword FROM elogin WHERE eusername='".$_POST['eusername']."' and epassword='".$_POST['epassword']."'");
   $row=$query->fetch_array();
   $count = $query->num_rows; // if email/password are correct returns must be 1 row
   if ($count==1) {
	   session_start();
	 $_SESSION['eSession'] = $row['eusername'];
